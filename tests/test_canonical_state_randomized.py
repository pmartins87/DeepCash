...
def random_snapshot(rng: random.Random, n: int) -> DecisionSnapshot:
    ...
    stage = rng.randrange(4)  # preflop, flop, turn, river
    flop = tuple(card_to_str(c) for c in sample[2:5]) if stage >= 1 else ()
    turn = card_to_str(sample[5]) if stage >= 2 else None
    river = card_to_str(sample[6]) if stage >= 3 else None
    ...
    history = [
        ActionSnapshot(
            ...
            pot_before=150,
            to_call_before=100,
            current_bet_before=100,
            actor_committed_before=0,
            min_full_raise_to_before=200,
        )
    ]
    if stage >= 1:
        history.append(
            ActionSnapshot(
                ...
                pot_before=575,
                to_call_before=75,
                current_bet_before=75,
                actor_committed_before=0,
                min_full_raise_to_before=150,
            )
        )
...
def test_randomized_2_to_6_player_metamorphic_invariance_battery():
    ...
    for n in range(2, 7):
        for _ in range(CASES_PER_PLAYER_COUNT):
            ...
            stages_seen.add((len(snapshot.flop), snapshot.turn is not None, snapshot.river is not None))
