...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "R4 Generation-2 private-state benchmark: deterministic matchup-profile "
            "clustering against an exact-combo common reference"
        )
    )
    ap.add_argument("--board-set", default="dev", choices=("dev", "heldout_v2"))
    ap.add_argument("--boards", default="all")
    ap.add_argument("--candidates", default="all")
    ap.add_argument("--range-combos", type=int, default=8)
    ap.add_argument("--p0-phase", type=float, default=0.19)
    ap.add_argument("--p1-phase", type=float, default=0.47)
    ...
    ap.add_argument("--checkpoints", default="300,1200,3600")
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("r4_generation2_representation.json"),
    )
    args = ap.parse_args()
    ...
    board_registry = gen2_board_registry(args.board_set)
    board_names = parse_names(args.boards, board_registry)
    available = {name: name for name in R4_GEN2_CANDIDATES}
    ...
    for board_name in board_names:
        ...
        for checkpoint in checkpoints:
            ...
            reference = representation_result_from_state(spec, exact_maps, reference_state)
            ...
            for candidate_name, item in candidates.items():
                ...
                row = {
                    ...
                    "method": "gen2_matchup_profile_clustering_one_sided_against_exact_combo_reference",
                    **bounds,
                }
                rows.append(row)
                print(
                    f"g2 {args.board_set:10s} {board_name:26s} {candidate_name:22s} "
    ...
    payload = {
        "schema": "DEEPCASH_R4_REPRESENTATION_GENERATION2_V1",
        "generation": 2,
        "method": (
            "exact cards/chance/payoffs; exact-combo common reference; deterministic legal-information "
            "matchup-profile clustering candidates; candidate private infosets aliased one player at a "
            "time; exact best responses after candidate expansion to combo identity"
        ),
        "selection_warning": (
            "Generation-2 development or held-out evidence never selects production by itself; "
            "physical Ryzen equal-wall-clock and street/stack-compatible validation remain mandatory"
        ),
        "board_set": args.board_set,
        ...
        "stack": args.stack,
        "spr": args.stack / args.pot,
        "reference_fractions": [str(value) for value in GEN2_REFERENCE_FRACTIONS],
        "bet_sizes": list(bet_sizes),
